...
public class SauceMakerBlockEntityRenderer implements BlockEntityRenderer<SauceMakerBlockEntity> {

    public static final ItemRenderer renderItem = MinecraftClient.getInstance().getItemRenderer();

    public SauceMakerBlockEntityRenderer(BlockEntityRendererFactory.Context context) {
    }
    ...
    public void render(SauceMakerBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if (!bottle.isEmpty() && state != Blocks.AIR.getDefaultState()){
            switch (state.get(SauceMakerBlock.FACING)){
    ...
    protected void renderItemForFacing(double x, double z, boolean ew, MatrixStack matrices, ItemStack stack, VertexConsumerProvider vertexConsumers, int light, int overlay, SauceMakerBlockEntity entity){
        matrices.push();
