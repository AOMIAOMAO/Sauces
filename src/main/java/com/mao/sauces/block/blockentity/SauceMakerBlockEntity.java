...
public class SauceMakerBlockEntity extends BlockEntity implements BlockEntityInv {
    public final RecipeType<SauceMakerRecipe> recipeType = SauceMakerRecipe.Type.INSTANCE;
    ...
    public SauceMakerBlockEntity(BlockPos pos, BlockState state) {
        super(EntityTypesRegistry.SAUCE_MAKER_BLOCK_ENTITY, pos, state);
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, SauceMakerBlockEntity entity) {
        ItemStack stack = entity.getItems().get(0);
        if (!stack.isEmpty()) {
            Optional<SauceMakerRecipe> optional = world.getRecipeManager().getFirstMatch(entity.recipeType, entity, world);
